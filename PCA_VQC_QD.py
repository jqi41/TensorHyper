...
In this notebook, we learn how to classify the charge stability diagram of single 
...
class PCA_VQC(nn.Module):
    def __init__(self, num_qubits=8, depth_vqc=1):
        super(PCA_VQC, self).__init__()
        self.flatten = nn.Flatten()
        self.num_qubits = num_qubits
        self.pca = PCA(n_components=8)
        self.vqc = MPS_VQC(n_wires=num_qubits, n_qlayers=depth_vqc, tensor_product_enc=True, add_fc=True)
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.1) # For each 10 iterations, the lr is divided by 2
